...
class YandexGPT:
    def generate_text(self, prompt):
        system_prompt = (
            ...
            "Я передаю тебе требования, которые актуальны в конкретном регионе, сформируй эти требования под нужный формат. "
            "КРИТИЧЕСКИ ВАЖНО: Верни данные строго в формате JSON без дополнительных символов, кавычек или markdown разметки. "
            "НЕ используй обратные кавычки, НЕ используй markdown блоки, НЕ добавляй никаких префиксов или суффиксов. "
            "Верни ТОЛЬКО валидный JSON объект, начинающийся с { и заканчивающийся на }. "
            "Формат ответа должен быть точно таким: "
            '{"soft_skills": [{"name": "Название навыка", "description": "Описание навыка (Нужно 3 строки)", "how_to_learn": "Краткое описание как освоить", "demand": "Процент спроса на рынке (только число)"}], "hard_skills": [{"name": "Название навыка", "description": "Подробное описание навыка (Нужно 6 строк)", "how_to_learn": "Пошаговое описание как освоить (нужно 5 строк)", "demand": "Процент спроса на рынке (только число)"}]}'
        )
